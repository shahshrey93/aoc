# learnings:
# - using named groups in regex can improve readability
# - generator expression in the all() function handles the early break implicitly. When all() encounters the first False value in the generator expression, it stops evaluating further and returns False
...
# %%
pattern = re.compile('Sue (?P<Sue_id>\d+): (?P<attr1>\w+): (?P<val1>-*\d+), (?P<attr2>\w+): (?P<val2>-*\d+), (?P<attr3>\w+): (?P<val3>-*\d+)')

input_dict = {int(match.group('Sue_id')):{match.group('attr1'):int(match.group('val1')), 
                                          match.group('attr2'):int(match.group('val2')), 
                                          match.group('attr3'):int(match.group('val3'))} 
...
filter_dict = {attr:int(val)
              for attr, val  in (pattern.match(line).groups() 
              for line in puzzle_input.splitlines())}
# %%
for Sue_id in input_dict:
    if all(
        (attr not in input_dict[Sue_id]) or 
        (attr in {'cats','trees'} and input_dict[Sue_id][attr]>filter_dict[attr]) or 
        (attr in {'pomeranians','goldfish'} and input_dict[Sue_id][attr]<filter_dict[attr]) or 
        (attr not in {'cats','trees','pomeranians','goldfish'} and input_dict[Sue_id][attr]==filter_dict[attr])
    for attr in filter_dict):
        print(Sue_id)
# %%
