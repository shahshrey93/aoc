...
# %%
light_status = np.zeros((1000, 1000), dtype=int)
...
for line in puzzle_input.splitlines():
    ...
    if line.startswith('turn on'):
        light_status[x1:x2+1, y1:y2+1] = light_status[x1:x2+1, y1:y2+1] + 1

    elif line.startswith('turn off'):
        light_status[x1:x2+1, y1:y2+1] = np.maximum(light_status[x1:x2+1, y1:y2+1] - 1, 0)

    elif line.startswith('toggle'):
        light_status[x1:x2+1, y1:y2+1] = light_status[x1:x2+1, y1:y2+1] + 2
    else:
        print(f'invalid line: {line}')
        sys.exit(1)

print(np.sum(light_status))
# %%